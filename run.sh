...
function generate-project {
    ...
    git add --all
    git branch -M main
    git commit -m "Initial commit, generated by cookiecutter"

}

# remove all files generated by tests, builds, or operating this codebase
function clean {
    rm -rf dist build coverage.xml test-reports sample tests_package/cookiecutter-*
    find . \
      -type d \
      \( \
        -name "*cache*" \
...
EOF
    ...
    git commit -m 'feat!: populated from `python-course-package(pagmaia)` template'
    ...
    gh pr create \
        --title "feat: populated from \`python-course-package(pagmaia)\` template" \
        --body "This PR was generated by \`python-course-package(pagmaia)\`" \
        --base main \
